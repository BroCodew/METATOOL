...
const PopupDetailAD = ({ dataDetail, detailParam }) => {
  ...
  function formatNumber(so) {
    let [phanNguyen, phanThapPhan] = parseFloat(so)
      .toFixed(2)
      .toString()
      .split(".");
  ...
  const handleReloadStorage = (e: any) => {
    ...
    chrome.runtime.sendMessage(
      { action: "reload_storage" },
      function (response) {}
    );
  };

  useEffect(() => {
    if (typeof dataAccount === "object" && dataAccount.length > 0) {
      let dataInfos = [];
      for (let i = 0; i < dataAccount.length; i++) {
        const total_spend =
          dataAccount[i].insights === null
            ? null
            : Number(dataAccount[i].insights?.data.map((item) => item.spend));
        const threshold_amount =
          dataAccount[i].adspaymentcycle === null
            ? null
            : Number(
                dataAccount[i]?.adspaymentcycle?.data
                  .map((item) => item.threshold_amount)
                  .join("")
              );
        const limit =
          dataAccount[i].adtrust_dsl === null
            ? null
            : Number(dataAccount[i].adtrust_dsl);
        //conver value to number
        const debtNumber =
          dataAccount[i].account_currency_ratio_to_usd <= 1
            ? dataAccount[i].balance / 100
            : +dataAccount[i].balance;
        const threNumber =
          dataAccount[i].account_currency_ratio_to_usd <= 1
            ? isNaN(threshold_amount)
              ? 0
        ...
        if (dataAccount[i].account_currency_ratio_to_usd <= 1) {
          ...
          threshold_usd = isNaN(
            threshold_amount / dataAccount[i].account_currency_ratio_to_usd
          )
            ? 0
            : threshold_amount / dataAccount[i].account_currency_ratio_to_usd;
        }
        ...
        const filterUserPermission = (i: any) => {
          return dataAccount[i]?.userpermissions.data.map(
            (item: any, i: any) => {
              if (item?.user?.id == detailParam) {
                return "Quản trị viên";
              } else return "";
            }
          );
        ...
        dataInfos.push({
          ...
          //Quyền tk
          PERMISSION_ACCOUNT: check ? "ADMIN" : "",
          ...
          //Role
          PERMISSION_BM: filterUserPermission(i),
          ...
          TIME_ZONE: `${
            dataAccount[i]?.timezone_offset_hours_utc > 0 ? "+" : ""
          }${dataAccount[i]?.timezone_offset_hours_utc} | ${
            dataAccount[i]?.timezone_name
          }`, //Múi giờ
        ...
        <div className="wrapper" id="main">
          <div className="sc_heading" style={{ padding: 0 }}>
            <div className="command">
              <div
                className="command_head"
                style={{ backgroundColor: "#2b3054" }}
              >
          ...
          <div className={styles.popupPosition}>
            <div
              ...
              style={{ maxHeight: `calc(100vh - 134px)`, height: "100vh" }}
            >
              ...
              >
                <thead id="thall">
                  <tr>
                    <th
                      className="sort"
                      onClick={() => handleSortItemNumber("STT")}
                    >
                      STT
                    </th>
                    <th
                      className="sort"
                      onClick={() => handleSortItemNumber("STATUS")}
                    >
                      Trạng thái
                    </th>
                    ...
                    <th
                      className="sort"
                      onClick={() => handleSortItemNumber("ID_TKQC_AD")}
                    >
                      ID
                    </th>
                    <th
                      className="sort"
                      onClick={() => handleSortItemText("NAME_TK_AD")}
                    >
                      Tên TK{" "}
                    </th>
                    <th
                      ...
                      onClick={() =>
                        handleSortItemNumber(
                          changeCurrency ? "DEBT" : "DEBT_USD"
                        )
                      }
                    >
                      Dư nợ
                    ...
                    <th
                      ...
                      onClick={() =>
                        handleSortItemNumber(
                          changeCurrency ? "THRESHOLD" : "THRESHOLD_USD"
                        )
                      }
                    >
                      Ngưỡng
                    ...
                    <th
                      ...
                      onClick={() =>
                        handleSortItemNumber(
                          changeCurrency ? "LIMIT" : "LIMIT_USD"
                        )
                      }
                    >
                      Limit
                    ...
                    <th
                      className="sort"
                      onClick={() => handleSortItemNumber("ADMIN")}
                    >
                      Admin
                    </th>
                    {/* <th
                      className="sort"
                      onClick={() => handleSortItemText("PERMISSION_ACCOUNT")}
                    >
                      Quyền TK
                    </th> */}
                    <th
                      className="sort"
                      onClick={() => handleSortItemText("CURRENCY")}
                    >
                      Tiền tệ
                    </th>
                    <th
                      className="sort"
                      onClick={() => handleSortItemText("ACCOUNT_TYPE")}
                    >
                      Loại TK
                    </th>
                    <th
                    ...
                    <th
                      className="sort"
                      onClick={() => handleSortItemText("PERMISSION_BM")}
                    >
                      Role
                    </th>
                    <th
                      className="sort"
                      onClick={() => handleSortItemNumber("ID_BM")}
                    >
                      ID BM
                    </th>
                    <th
                    ...
                    <th
                      className="sort"
                      onClick={() => handleSortPaymentMethod("PAYMENT_METHOD")}
                    >
                      Thanh toán
                    </th>
                    <th
                      className="sort"
                      onClick={() => handleSortItemNumber("TIME_ZONE")}
                    >
                      Múi giờ
                    </th>
                  </tr>
                  ...
                  {filteredList.map((item, key) => (
                    <tr className="trInfo" key={key}>
                      ...
                      <td className="tdInfo">
                        <span className="r">
                          {changeCurrency
                            ...
                            : item.THRESHOLD_USD <= 0
                            ? "--"
                            : formatNumber(item.THRESHOLD_USD)}
                        </span>
                      </td>
                      <td className="tdInfo">
                        <span className="r">
                      ...
                      <td className="tdInfo">
                        <span className="r">
                          {changeCurrency
                            ? formatNumber(item.TOTAL_SPENDING)
                            : formatNumber(item.TOTAL_SPENDING_USD)}
                        </span>
                      </td>
                      <td className="tdInfo">
                      ...
                      <td className="tdInfo" style={{ textAlign: "left" }}>
                        {item.TIME_ZONE}
                      </td>
                    </tr>
                  ))}
              ...
              <table id="table" className={styles.sticky}>
                <tbody>
                  <tr>
                    <td
                      style={{
                        width: `${widthMain.widthTotal}px`,
                    ...
                    >
                      $
                      {formatNumber(
                        infos.reduce((total, item) => total + item.DEBT_USD, 0)
                      )}
                    </td>
                    <td
                      className="tdInfo"
                    ...
                    >
                      ...
                      {formatNumber(
                        infos.reduce(
                          (total, item) => total + item.THRESHOLD_USD,
                          0
                        )
                      )}
                    </td>
                    ...
                    >
                      ...
                      {formatNumber(
                        infos.reduce(
                          (total, item) => total + item.TOTAL_SPENDING_USD,
                          0
                        )
                      )}
                    </td>
